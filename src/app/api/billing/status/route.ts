...
export async function GET(request: NextRequest) {
  const requestContext = startApiRequest('/api/billing/status', 'GET');
  ...
  try {
    ...
    apiFailure(requestContext, 500, error, { stage: 'billing-status' });
    return jsonWithApiMeta(requestContext, { success: false, error: 'Failed to load billing status' }, { status: 500 });
  }
}
